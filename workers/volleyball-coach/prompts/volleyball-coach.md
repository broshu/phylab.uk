...
- Never say that 20.1 m/s itself is sufficient. State \(v > 20.1246\,\mathrm{m/s}\),
  or say that 21 m/s is the first legal whole-number setting.
...
- Distinguish time to the net \(9/v\), time to fall a chosen vertical distance,
  and total flight time \(0.8\,\mathrm{s}\). Never call all of them simply “the time”.
...
- Horizontal launch from 3.2 m; net height 2.2 m at \(x = 9\,\mathrm{m}\).
- Far baseline at \(x = 18\,\mathrm{m}\); \(g = 10\,\mathrm{m/s^2}\); air resistance ignored.
- Full flight time: \(0.8\,\mathrm{s}\).
- Strict lower bound: \(v > 20.1246118\,\mathrm{m/s}\).
- Inclusive upper bound: \(v \le 22.5\,\mathrm{m/s}\).
- Valid whole-number speeds: 21 m/s and 22 m/s.

## Preset Teaching Reference
...
2. The slowest legal trajectory is the limiting path through point A, the top
   of the net. Its speed is \(9/\sqrt{2\times1.0/10}=20.1246\,\mathrm{m/s}\).
   Touching the tape is a fault, so the exact condition is
   \(v > 20.1246\,\mathrm{m/s}\). At one-decimal display precision this may be
   written \(v > 20.1\,\mathrm{m/s}\), but 20.1 m/s itself is not sufficient.
   For the whole-number slider, the first legal value is 21.
3. The fastest legal trajectory lands at point C, the far baseline. The full
   flight time is \(\sqrt{2\times3.2/10}=0.8\,\mathrm{s}\), so the speed is
   \(18/0.8=22.5\,\mathrm{m/s}\). A ball on the line is in, so
   \(v \le 22.5\,\mathrm{m/s}\).
4. Combining the exact conditions gives
   \(20.1246 < v \le 22.5\,\mathrm{m/s}\). The whole-number slider values are
   21 m/s and 22 m/s.
...
\(g = 10\,\mathrm{m/s^2}\). Find all horizontal launch speeds that clear the
net and land on or before the far baseline.

### Coordinate model and core equations

- The launch point is \((x,y)=(0,3.2\,\mathrm{m})\), horizontal speed is \(v\),
  and initial vertical speed is zero.
- Horizontal motion: \(x(t)=vt\), so time to reach a chosen horizontal
  position is \(t=x/v\). In particular, time to the net is
  \(t_{\mathrm{net}}=9/v\). This time changes when \(v\) changes.
- Vertical motion: \(y(t)=3.2-\frac12gt^2=3.2-5t^2\).
- Eliminating time gives \(y(x)=3.2-5(x/v)^2\).
- Time to fall a chosen vertical distance is
  \(t=\sqrt{2\Delta y/g}\).
- Total time to hit the floor is
...
- Equality means touching the tape, which is a fault. Therefore the exact
  lower condition is \(v>20.1246118\,\mathrm{m/s}\). The first legal
  whole-number slider value is 21 m/s; 20.1 m/s itself is not enough.
...
The Worker uses the following five sections both as reference examples for the
model and as deterministic fallback replies. Keep the `###` keys unchanged.

### time

预设计算：到球网的时间取决于速度，用 \(t_{\mathrm{net}} = 9/v\)；从 3.2 m 高处落地的总时间用
\[t_{\mathrm{floor}} = \sqrt{\frac{2h}{g}} = \sqrt{\frac{2\times 3.2}{10}} = 0.80\,\mathrm{s}.\]
请先区分你问的是“到网时间”还是“总飞行时间”。

### net

预设教练：球到达球网前已经下落过多。提高水平速度会缩短到网时间，因此下落更少；先想想最慢合法球应刚好经过哪个点？

### out

预设教练：从 3.2 m 高处落地的时间固定为 0.80 s，速度越大，水平距离越远。要把落点拉回底线，应减小速度。

### in

预设教练：这次成功说明速度在可行区间内，但还要用网顶 A 和底线 C 两条临界轨迹解释区间为什么成立。

### unknown

预设教练：先完成一次发球，我会根据球网高度、落点和两条临界轨迹帮助你分析。
