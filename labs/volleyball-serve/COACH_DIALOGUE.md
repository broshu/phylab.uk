...
flowchart TD
    ...
    MaxRest --> Final[同时满足还是满足其一 → 合并区间 → 多选整数速度]
    MinRest --> Final
...
| `demo` | 隐藏速度，播放刚好到达该点的临界发球 | 无 | 只保留该点；显示水平距离与竖直下落的虚线，数值先显示为「?」；球员完成发球动作 | 水平距离 |
| `distance` | 「从击球点到该点，水平距离是多少？」 | **输入**（A：9 m；C：18 m） | C 输入 9：「9 m 只到球网，C 在对方底线」（E7）；A 输入 18 同理；其他错值：提示沿水平虚线看。第二次仍错：直接给出数值 | 虚线标出距离 |
| `fall` | 「从击球点到该点，竖直下落多少？」 | A：**输入** 1.0 m；C：**三选一** 1.0 / 2.2 / 3.2 m | A 输入 2.2：「2.2 m 是 A 离地的高度，不是下落高度」（E2）；A 输入 3.2：那是落到地面的高度。C 选 1.0 / 2.2 同样分别解释。第二次仍错：直接给出数值 | 虚线标出下落高度 |
| `time` | 「下落这段高度要多久？」 | **三选一** 0.45 / 0.66 / 0.80 s | A 选 0.80 s：「这是落到地面的时间，球到 A 时只下落了 1.0 m」（E1）；选 0.66 s：「这是下落 2.2 m（网高）的时间」（E2）。C 选 0.45 / 0.66 同理。第一次答错后显示公式 \(t=\sqrt{2\Delta h/g}\) 并代入（不算出结果）；再问一次 | 求速度 |
| `calculate` | 「刚好到达该点的球速是多少？」 | **三选一**，三个候选分别是水平距离除以上面三个时间（A：11.25 / 13.6 / 20.1 m/s；C：22.5 / 27.1 / 40.2 m/s） | 第一次答错：用 KaTeX 展示两步计算；再问一次 | 写出不等式 |
| `rule` | `min`：碰网算失误，边界严格，`v > vMin`；`max`：压线有效，边界含等号，`v ≤ vMax` | 无 | 无 | 标记该端完成 |

**输入还是选择**：由正确答案决定——正确值是整数（9 m、18 m、1.0 m）就让学生输入，不是整数（3.2 m、0.45 s、20.1 m/s）就给选项。这样学生不会因为四舍五入到几位而被判错，判的只是物理。规则写在 `askValue()` 里，换题目时自动跟随。输入框接受「9」「9.0」「9 m」和全角数字；不是数字的输入会原地提示，不算一次作答。
...
| 节点 | Coach 行为／文案意图 | 学生输入 | 纠错／画面 | 后续 |
| --- | --- | --- | --- | --- |
| `interval.combine` | 列出两个条件，问「合法发球要同时满足两个条件，还是满足一个就够？」 | 同时满足 / 满足一个就够 | 选「满足一个就够」：演示 16 m/s（满足 `v ≤ vMax` 但挂网）和 25 m/s（满足 `v > vMin` 但出界），再问一次（E4/E5） | 写出区间 |
| `interval.final` | 写成 `vMin < v ≤ vMax` | 无 | 无 | 多选整数速度 |
...
| `lessonStep` | `diagnose` / `point` / `demo` / `distance` / `fall` / `time` / `calculate` / `rule` / `speeds` / `points` / `combine` / `final` / `none` |
